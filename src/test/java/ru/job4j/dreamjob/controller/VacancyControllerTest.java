...
public class VacancyControllerTest {
    ...
    @Test
    public void whenVacancyGetByIdThenReturnVacancy() {
        var vacancyOptional = Optional.of(
                new Vacancy(1, "test1", "desc1", now(), true, 1, 2));
        when(vacancyService.findBy(anyInt())).thenReturn(vacancyOptional);

        var model = new ConcurrentModel();
        var view = vacancyController.getById(model, vacancyOptional.get().getId());
        var actualVacancy = model.getAttribute("vacancy");

        assertThat(view).isEqualTo("vacancies/one");
        assertThat(actualVacancy).usingRecursiveComparison().isEqualTo(vacancyOptional.get());
    }

    @Test
    public void whenVacancyGetByIdThenGetErrorPageWithMessage() {
        when(vacancyService.findBy(anyInt())).thenReturn(Optional.empty());
    ...
    @Test
    public void whenUpdateVacancyThenReturnVacanciesPage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
    ...
    @Test
    public void whenUpdateVacancyThenReturnErrorPageWithMessage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
    ...
    @Test
    public void whenUpdateVacancyThenThrowExceptionReturnErrorPageWithMessage() {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        when(vacancyService.update(any(), any())).thenThrow(new RuntimeException());
    ...
    @Test
    public void whenDeleteVacancyThenGetVacanciesPage() {
        int id = 1;
        when(vacancyService.delete(anyInt())).thenReturn(true);
    ...
    @Test
    public void whenDeleteVacancyThenGetErrorPageWithMessage() {
        int id = 1;
        when(vacancyService.delete(anyInt())).thenReturn(false);
        ...
        assertThat(view).isEqualTo("errors/404");
        assertThat(message).isEqualTo("Вакансия с указанным идентификатором не найдкна");
    }
